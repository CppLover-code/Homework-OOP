...
public:   
    ...
    MyString(MyString&& s) noexcept                      // конструктор перемещения
    {
...
int main()
{
    ...
    MyString str5(s1); // сработал конструктор 3
    MyString str6(s2); // сработал конструктор 3
    MyString RES = str5 + str6; // для временного объекта внутри перегрузки опертора + сработал конструктор 1, а при возврате temp в место вызова - сработал конструктор перемещения
    RES.Output();
...
/*
    Заметки для меня:
    В ходе работы программы обратила внимание, что без конструктора перемещения при возврате экземпляра класса temp в место вызова, например, 
    MyString NEW3 = D + str11; или MyString RES = str5 + str6; (в перегрузке оператора происходит возврат временного объекта temp)
    срабатывает КОНСТРУКТОР КОПИРОВАНИЯ (внутри происходит лишняя затрата ресурсов).
    А при появлении КОНСТРУКТОРА ПЕРЕМЕЩЕНИЯ вместо конструктора копирования начало работать именно перемещение, то есть при возврате
    временного объекта из перегрузки новосозданному объекту в main(), например, MyString NEW3 - передались права владения РЕСУРСАМИ объекта,
    который был возвращен (temp). 
*/